...
void term_resize_check()
{
    if ((COLS == dgl_local_COLS) && (LINES == dgl_local_LINES) && !curses_resize)
        return;
        ...
        if (ispercent)
        {
            ...
            case 'g':
                if (game >= 0 && game < num_games && myconfig[game])
                    snprintf(p, end + 1 - p, "%s", myconfig[game]->game_name);
                else
                    return NULL;
                while (*p != '\0')
                    p++;
            ...
            case 's':
                if (game >= 0 && game < num_games && myconfig[game])
                    snprintf(p, end + 1 - p, "%s", myconfig[game]->shortname);
                else
                    return NULL;
                while (*p != '\0')
                    p++;
...
int dgl_exec_cmdqueue(struct dg_cmdpart *queue, int game, struct dg_user *me)
{
    ...
    {
        if (tmp->param1)
            strcpy(p1, dgl_format_str(game, me, tmp->param1, NULL));
        if (tmp->param2)
            strcpy(p2, dgl_format_str(game, me, tmp->param2, NULL));
        ...
        case DGLCMD_LOGIN:
            if (!loggedin)
                loginprompt(0);
            if (loggedin)
                runmenuloop(dgl_find_menu(get_mainmenu_name()));
            break;
...
sort_games(struct dg_game **games, int len, dg_sortmode sortmode)
{
    ...
    case SORTMODE_USERNAME:
        qsort(games, len, sizeof(struct dg_game *), sort_game_username);
        break;
    case SORTMODE_GAMENUM:
        qsort(games, len, sizeof(struct dg_game *), sort_game_gamenum);
        break;
    case SORTMODE_WINDOWSIZE:
        qsort(games, len, sizeof(struct dg_game *), sort_game_windowsize);
        break;
    ...
    case SORTMODE_STARTTIME:
        qsort(games, len, sizeof(struct dg_game *), sort_game_starttime);
        break;
...
void free_populated_games(struct dg_game **games, int len)
{
    ...
    for (i = 0; i < len; i++)
    {
        if (games[i]->ttyrec_fn)
            free(games[i]->ttyrec_fn);
        if (games[i]->name)
            free(games[i]->name);
        if (games[i]->date)
            free(games[i]->date);
        if (games[i]->time)
            free(games[i]->time);
        if (games[i]->extra_info)
            free(games[i]->extra_info);
        free(games[i]);
    }
    free(games);
}
...
void create_config()
{
    ...
    if (!globalconfig.chroot)
        globalconfig.chroot = "/var/lib/dgamelaunch/";

    if (globalconfig.max == 0)
        globalconfig.max = 64000;
    if (globalconfig.max_newnick_len == 0)
        globalconfig.max_newnick_len = DGL_PLAYERNAMELEN;
    if (!globalconfig.dglroot)
        globalconfig.dglroot = "/dgldir/";
    if (!globalconfig.banner)
        globalconfig.banner = "/dgl-banner";

    if (!globalconfig.passwd)
        globalconfig.passwd = "/dgl-login";
    if (!globalconfig.lockfile)
        globalconfig.lockfile = "/dgl-lock";
