...
void idle_alarm_set_enabled(int enabled)
{
...
void loadbanner(char *fname, struct dg_banner *ban)
{
  FILE *bannerfile;
  char buf[80];
  if (ban->len > 23)
...
void drawbanner(struct dg_banner *ban, unsigned int start_line, unsigned int howmany)
{
  unsigned int i;
...
void shm_sem_wait(struct dg_shm *shm_dg_data)
{
#ifdef USE_SHMEM
...
void shm_sem_post(struct dg_shm *shm_dg_data)
{
#ifdef USE_SHMEM
...
void shm_update(struct dg_shm *shm_dg_data, struct dg_game **games, int len)
{
#ifdef USE_SHMEM
  int di, i;
...
void shm_init(struct dg_shm **shm_dg_data, struct dg_shm_game **shm_dg_game)
{
#ifdef USE_SHMEM
  key_t shm_key;
  key_t shm_sem_key;
...
void inprogressdisplay(int gameid)
{
...
void wall_email(char *from, char *msg)
{
  int len, i;
  struct dg_game **games = NULL;
  ...
  for (i = 0; i < len; i++)
  {
    ...
    int fnamelen;
    if (strlen(myconfig[game]->spool) < 1)
      continue;
...
void autologin(char *user, char *pass)
{
  struct dg_user *tmp;
...
void loginprompt(int from_ttyplay)
{
...
void write_canned_rcfile(int game, char *target)
{
  FILE *canned, *newfile;
  char buf[1024], *rfn;
  size_t bytes, len;
...
int purge_stale_locks(int game)
{
  DIR *pdir;
  struct dirent *dent;
  char *dir;
...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt(argc, argv, "csqh:pi:aeW:SD")) != -1)
  {
    ...
    case 'S': /* Free the shared memory block */
      ...
      {
        if (!silent)
          fprintf(stderr, "nonexistent shmem block.\n");
      }
      else
      {
        if (!silent)
          fprintf(stderr, "shmem block freed.\n");
      }
#else
      if (!silent)
        fprintf(stderr, "warning: dgamelaunch was compiled without shmem.\n");
#endif
      graceful_exit(0);
      break;
